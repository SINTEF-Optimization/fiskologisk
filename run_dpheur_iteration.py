...
if __name__ == "__main__":
    ...
    avg_tank_volume = sum(1.0 / t.inverse_volume for t in environment.tanks) / len(environment.tanks)
    ...
    for dep_p in environment.release_periods:
        ...
        for p in dep_p.periods_after_deploy:
            ...
            print(f"   - p={p.index} G={data.growth_factor} G^T={data.transferred_growth_factor}",
                  ...
                  f"POSTSMOLT p={post_smolt_sell_price[dep_p.index][p.index]}" if p in dep_p.postsmolt_extract_periods else "",
